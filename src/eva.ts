/**
 * EVA Engine - Coherence Validation
 * Source: R11 Book II S.II.6, Book III S.III.A.3.3
 *
 * The EVA Engine is:
 *   - Deterministic (same inputs always produce same outputs)
 *   - Side-effect free (reads inputs, produces outputs, modifies nothing)
 *   - Non-participating (does not originate intent or hold preferences)
 *   - Binary (VALID or INVALID, no partial credit)
 */

import {
  PHI,
  EPSILON_0,
  GAMMA_MIN,
  GAMMA_BLOOM,
  GAMMA_ROOT,
  ALPHA_DEFAULT,
  Classification,
  Verdict,
} from './constants';

/** Raw EVA inputs */
export interface EVAInput {
  /** Energy commitment [0, 1] - normalised thermodynamic expenditure */
  E: number;
  /** Vector alignment [0, 1] - cosine similarity between intent and action */
  V: number;
  /** Attention persistence [0, 1] - negentropy measure */
  A: number;
  /** Temporal friction [0, infinity) - accumulated drift and resistance */
  tau: number;
}

/** EVA Engine output */
export interface EVAResult {
  /** Coherence Index [0, 1] */
  gamma: number;
  /** Causal Time Units generated (0 if invalid) */
  ctu: number;
  /** Entropy delta (1 - gamma, must be > 0) */
  deltaS: number;
  /** Classification tier */
  classification: Classification;
  /** Binary verdict */
  verdict: Verdict;
  /** Temporal debt (only if verdict is INVALID) */
  temporalDebt: number;
}

/**
 * Compute the Coherence Index (Gamma).
 *
 * Formula: Gamma = (E * V * A) / (tau + epsilon_0)
 * Source: Book I S.I.7.1, Book II S.II.4.1
 *
 * Multiplicative structure is deliberate: if any single component
 * is zero, Gamma is zero. Energy without direction produces nothing.
 * Direction without energy produces nothing. Neither without sustained
 * attention produces anything.
 */
export function computeGamma(input: EVAInput): number {
  const { E, V, A, tau } = input;
  const gammaRaw = (E * V * A) / (tau + EPSILON_0);
  return Math.min(1.0, Math.max(0.0, gammaRaw));
}

/**
 * Compute Causal Time Units.
 *
 * Formula: CTU = phi * E * V * A
 * Source: Book II S.II.5.1
 *
 * CTU is generated only when verdict is VALID.
 */
export function computeCTU(E: number, V: number, A: number): number {
  return PHI * E * V * A;
}

/**
 * Classify a Gamma value into coherence tiers.
 *
 * Source: Book II S.II.4.2
 *
 * SEED:     [0.70, 0.8187)  - Carnot efficiency limit
 * BLOOM:    [0.8187, 0.95)  - Landauer erasure threshold
 * ROOT:     [0.95, 1.00)    - Relativistic threshold
 * REJECTED: [0, 0.70)       - Below measurable coherence
 *
 * Note: Gamma = 1.0 is thermodynamically impossible (Second Law).
 */
export function classify(gamma: number): Classification {
  if (gamma >= GAMMA_ROOT) return Classification.ROOT;
  if (gamma >= GAMMA_BLOOM) return Classification.BLOOM;
  if (gamma >= GAMMA_MIN) return Classification.SEED;
  return Classification.REJECTED;
}

/**
 * Compute temporal debt for failed validations.
 *
 * Formula: D = phi * (Gamma_min - Gamma) * E
 * Source: Book II S.II.7.2
 *
 * Only fires when Gamma < Gamma_min.
 */
export function computeTemporalDebt(gamma: number, E: number): number {
  if (gamma >= GAMMA_MIN) return 0;
  return PHI * (GAMMA_MIN - gamma) * E;
}

/**
 * Compute Attention from variance.
 *
 * Formula: A = 1 / (1 + alpha * sigma^2)
 * Source: Book VI S.VI.B.3
 *
 * As variance increases (more noise), Attention approaches zero.
 * As variance decreases (more focus), Attention approaches 1.
 */
export function computeAttention(
  gammaVariance: number,
  alpha: number = ALPHA_DEFAULT
): number {
  return 1 / (1 + alpha * gammaVariance);
}

/**
 * Full EVA Engine validation.
 *
 * This is the atomic validation unit of CTP/IP.
 * Source: Book II S.II.6.1, Book III S.III.A.3.3
 *
 * O(1) per PoT (Theorem 5, S.III.A.7):
 * 3 input clamps, 1 multiplication, 1 addition, 1 division,
 * 1 subtraction, 1 multiplication, 2 comparisons = 10 fixed-size
 * arithmetic operations.
 */
export function evaluateEVA(input: EVAInput): EVAResult {
  // Clamp inputs to [0, 1]
  const E = Math.min(1, Math.max(0, input.E));
  const V = Math.min(1, Math.max(0, input.V));
  const A = Math.min(1, Math.max(0, input.A));
  const tau = Math.max(0, input.tau);

  // Coherence Index
  const gamma = computeGamma({ E, V, A, tau });

  // Entropy delta (must be > 0; Gamma = 1.0 is impossible)
  const deltaS = 1 - gamma;

  // Classification
  const classification = classify(gamma);

  // Binary verdict
  const verdict = gamma >= GAMMA_MIN && deltaS > 0
    ? Verdict.VALID
    : Verdict.INVALID;

  // CTU (only on valid)
  const ctu = verdict === Verdict.VALID ? computeCTU(E, V, A) : 0;

  // Temporal debt (only on invalid)
  const temporalDebt = verdict === Verdict.INVALID
    ? computeTemporalDebt(gamma, E)
    : 0;

  return { gamma, ctu, deltaS, classification, verdict, temporalDebt };
}
